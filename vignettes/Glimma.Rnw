...
Last revised \cleanlookdateon \today
}
\end{center}
\newpage
\tableofcontents
...
Glimma's interactive plots can be distributed to others by sharing the ``glimma-plots" folder and its contents, and linked to in Rmarkdown analysis reports.
...
# BUG regarding highlight colors in MD plot
# BUG regarding not being able to work properly without anno or samples
