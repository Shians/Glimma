...
<<Rarraydata, echo=FALSE, eval=FALSE>>=
...
<<Rmd.arrays, eval=FALSE>>=
# MD Plot
glMDPlot(fit, status=dt, counts=arrays$E, groups=targets$Condition,
  id.column="ProbeID", side.ylab="log-expression", transform=FALSE,
  main=colnames(fit)[ncol(fit)])
@

XY-Plot

<<Rxy.arrays, eval=FALSE>>=
...
glXYPlot(x=table1$logFC, y=table2$logFC, side.ylab="log-expression", transform=FALSE,
  ...
  anno=table1[,2:5], id.column="ProbeID")
@
